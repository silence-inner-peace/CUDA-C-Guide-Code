...
void useStream(float *sub_dA, float *sub_hA, float *sub_dB, float *sub_hB, size_t iBytes,int iElem, 
dim3 grid, dim3 block, cudaStream_t &streami, float *sub_dC, float* gpuRef)
{
    CHECK(cudaMemcpyAsync(sub_dA, sub_hA, iBytes,
                          cudaMemcpyHostToDevice, streami));
    CHECK(cudaMemcpyAsync(sub_dB, sub_hB, iBytes,
                          cudaMemcpyHostToDevice, streami));
    sumArrays<<<grid, block, 0, streami>>>(sub_dA, sub_dB,
            sub_dC, iElem);
    CHECK(cudaMemcpyAsync(gpuRef, sub_dC, iBytes,
                          cudaMemcpyDeviceToHost, streami));
...
int main(int argc, char **argv)
{
    ...
    {
        int ioffset = i * iElem;
        // useStream(&d_A[ioffset], &h_A[ioffset],&d_B[ioffset], &h_B[ioffset],iBytes,iElem,grid,block,stream[i],&d_C[ioffset],&gpuRef[ioffset]);
